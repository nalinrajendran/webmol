/*! MolView JavaScript Sketcher build on 2020-07-30 */
...
var MPFragments = {
    ...
    init: function() {
        this.benzene = this.generateRing(6, !0), this.cyclopropane = this.generateRing(3, !1), this.cyclobutane = this.generateRing(4, !1), this.cyclopentane = this.generateRing(5, !1), this.cyclohexane = this.generateRing(6, !1), this.cycloheptane = this.generateRing(7, !1)
    },
...
function MPAtom(t, e) {
    this.mp = t, this.index = e.i, this.center = new MPPoint(e.x || 0, e.y || 0), this.element = e.element || "C", this.charge = e.charge || 0, this.isotope = e.isotope || 0, this.bonds = void 0 !== e.bonds ? e.bonds.slice() : [], this.selected = e.selected || !1, this.display = "normal", this.valid = !1, this.mp.requestRedraw()
}
MPAtom.prototype.getX = function() {
    ...
    var e = this.bonds.indexOf(t); - 1 !== e && (this.bonds.splice(e, 1), this.bondsChanged())
    ...
    return "atom" === this.mp.tool.type ? {
        ...
        onPointerDown: function(t, e) {
            ...
            }, e.sel.clear(), e.sel.currentAngle = e.sel.startAngle = this.scope.calculateNewBondAngle(), e.sel.center = this.scope.center.clone(), MPFragments.rotate(this.data.frag, this.scope.center, e.sel.currentAngle);
...
function MPBond(t, e) {
    this.mp = t, this.index = e.i, this.type = e.type || 0, this.stereo = e.stereo || MP_STEREO_NONE, this.from = e.from || 0, this.to = e.to || 0, this.selected = e.selected || !1, this.display = "normal", this.hidden = !1, this.valid = !1, this.mp.requestRedraw()
}
...
function MolPad(t, e, o) {
    this.loadSettings(), this.tool = {
        ...
        selection: []
    }, this.mol = new MPMolecule(this), this.sel = new MPSelection(this), this.buttons = o, this.container = jQuery(t), this.offset = this.container.offset(), this.devicePixelRatio = e || 1, this.setupEventHandling(), this.setupGraphics()
}

function MPMolecule(t) {
    this.atoms = [], this.bonds = [], this.stack = [], this.reverseStack = [], this.copy = {
        atoms: [],
        bonds: [],
        fingerprint: ""
    }, this.mp = t
...
MolPad.prototype.setTool = function(t, e) {
    ...
    jQuery(this.buttons.undo).toggleClass("tool-button-disabled", 0 === this.mol.stack.length), jQuery(this.buttons.redo).toggleClass("tool-button-disabled", 0 === this.mol.reverseStack.length), void 0 !== this.changeCallback && this.changeCallback()
    ...
    this.mp.mol.exec(function(t) {
        t.selected = !1
    }, !0, !0), this.center = void 0, this.update(), this.mp.requestRedraw()
    ...
    this.canvas = document.createElement("canvas"), this.canvas.width = this.container.width() * this.devicePixelRatio, this.canvas.height = this.container.height() * this.devicePixelRatio, this.canvas.style.width = this.container.width() + "px", this.canvas.style.height = this.container.height() + "px", this.container.append(this.canvas), this.redrawRequest = !1, this.matrix = [1, 0, 0, 1, 0, 0], this.ctx = this.canvas.getContext("2d"), this.updated = !1, this.pendingFrame = !1
}, MolPad.prototype.resize = function() {
    this.canvas.width = this.container.width() * this.devicePixelRatio, this.canvas.height = this.container.height() * this.devicePixelRatio, this.canvas.style.width = this.container.width() + "px", this.canvas.style.height = this.container.height() + "px", this.offset = this.container.offset(), this.center()
    ...
    this.pendingFrame = !1, this.updated || (this.updated = !0, this.update()), this.mol.validateAll(), this.ctx.clearRect(0, 0, this.width(), this.height()), this.ctx.save(), this.ctx.transform(this.matrix[0], this.matrix[1], this.matrix[2], this.matrix[3], this.matrix[4], this.matrix[5]), this.ctx.lineWidth = 2 * this.s.bond.radiusScaled, this.ctx.lineCap = this.s.bond.lineCap;
    ...
    this.container.on("DOMMouseScroll mousewheel", function(t) {
        ...
        e.onBlur(t)
    }), 0 <= navigator.platform.toLowerCase().indexOf("mac") ? (jQuery(document).bind("keydown", "meta+z", function(t) {
    ...
    return "atom" === this.tool.type ? {
        ...
        onPointerDown: function(t, e) {
            ...
            e.mol.bonds.push(n), i.addBond(n.index), s.addBond(n.index), e.pointer.handler.scope = n, e.sel.update(), e.sel.center = o.clone(), e.sel.currentAngle = e.sel.startAngle = .5 * Math.PI
        },
        onPointerMove: function(t, e) {
            var o = (new MPPoint).fromRelativePointer(t, e);
            e.sel.rotate(o)
